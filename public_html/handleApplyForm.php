<?php
	...
	$_POST['city'], $_POST['state'], $_POST['zipCode'],$_POST['vegan'], $_POST['vegetarian'], $_POST['organiaztion'],  $_POST['currentTitle'], $_POST['sponsorName'], 
	...
	$organitaztion = filter_input(INPUT_POST, 'organiaztion', FILTER_SANITIZE_SPECIAL_CHARS);
	...
	try 
	{
		...
		*/
        ...
        'diet'                   => trim((string)($_POST['dietaryRestrictions'] ?? '')),
        // HTML "accessibilityNeeds" → expected "accommodations"
        'accommodations'         => trim((string)($_POST['accessibilityNeeds'] ?? '')),
		
        // HTML "organiaztion" (sic) → expected "org"
        'org'                    => trim((string)($_POST['organiaztion'] ?? '')),
        // HTML "currentTitle" → expected "title"
        'title'                  => trim((string)($_POST['currentTitle'] ?? '')),
		
        // Sponsor / supervisor fields
        // HTML "sponsorName" → expected "supName"
        'supName'                => trim((string)($_POST['sponsorName'] ?? '')),
        // HTML "sponsorEmail" → expected "supEmail"
        'supEmail'               => trim((string)($_POST['sponsorEmail'] ?? '')),
        // HTML "sponsorPhone" → expected "supPhone"
        'supPhone'               => trim((string)($_POST['sponsorPhone'] ?? '')),
		
        'refferalQuestion'       => trim((string)($_POST['refferalQuestion'] ?? '')),
        ...
        'partialScholarship'     => trim((string)($_POST['partialScholarship'] ?? '')),
        'assistAmount'           => trim((string)($_POST['assistAmount'] ?? '')),
		];
